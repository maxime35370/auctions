...
| V2.4 | **🃏 Lots de cartes Pokémon** : identification par numéro/collection (API publique pokemontcg.io, prix Cardmarket € — exception assumée au principe « pas d'API » : enrichissement optionnel, jamais le socle), valeur totale, cartes à forte valeur, **estimation volontairement prudente** (illisibles × valeur plancher), **modes de confiance honnêtes** 🟢 précis / 🟡 fourchette / 🔴 « je ne peux pas estimer précisément » plafonnés par la lisibilité déclarée des photos, bascule en analyse pré-remplie | ✅ livré |
| V2.5 | OCR des photos (Tesseract.js) pour lire numéros de cartes et qualité photo automatique ; autres jeux (Magic, Yu-Gi-Oh) ; connecteur Interencheres affiné sur pages réelles ; Agorastore, Enchères-Domaine ; risque mesuré (traçage des réparations) ; délais moyens par plateforme | à venir |
